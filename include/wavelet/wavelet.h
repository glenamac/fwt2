...
void (*filter)(double **c, double **d,int *support);
...
void initD4(double **c, double **d, int *support)
{
    ...
    double norm  = 4.0 * sqrt(2.0);  // normalization factor = 4·√2

    // low-pass (scaling) coefficients
    D4c[0] = (1.0 + sqrt3) / norm;
    D4c[1] = (3.0 + sqrt3) / norm;
    D4c[2] = (3.0 - sqrt3) / norm;
    D4c[3] = (1.0 - sqrt3) / norm;
...
{
    double sqrt3 = sqrt(3.0);
    double norm  = 4.0 * sqrt(2.0);  // normalization = 4·√2

    // forward (analysis) low-pass taps
    double fwd[4] = {
        (1.0 + sqrt3) / norm,
        (3.0 + sqrt3) / norm,
        (3.0 - sqrt3) / norm,
    ...
    // synthesis low-pass is time-reversed
    for (int i = 0; i < 4; i++) {
        D4c[i] = fwd[3 - i];
    }

    // synthesis high-pass via QMF: g[k] = (–1)^k · h[N–1–k]
    for (int k = 0; k < 4; k++) {
        D4d[k] = ((k & 1) ? -1.0 : 1.0) * D4c[3 - k];
    }
